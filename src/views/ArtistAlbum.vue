				...
				<div class="apps-card">
					<div class="app-song-detail song-list-wrap">
						<div class="cover-img-size">
							<img :src="singer.picUrl" alt=" " class="mv-img" />
						</div>
						<div class="song-info">
							...
							<p class="introduction">简介：{{ introduction }}</p>
						</div>
					</div>
				</div>
			</div>
...
export default {
	...
	methods: {
		...
		getArtistAlbum(page = 1) {
			getArtistAlbumApi({
				id: this.$route.query.singerId,
				limit: this.limit,
				offset: (page - 1) * this.limit,
			}).then((response) => {
				this.singer = response.data.artist;
				this.introduction = this.singer.alias[0];
			});
		},
		getArtistSongs() {
			getArtistSongsApi(this.$route.query.singerId).then((response) => {
